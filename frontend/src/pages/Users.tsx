...
import { createAdUser, deleteAdUser, getAdUsers, updateAdUser } from "../services/api";
...
export default function Users() {
	...
	async function handleSubmit(input: AdUserInput) {
		if (editingUser) {
			...
			setAdUsers((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
			return;
		}
	...
	async function handleDelete(adUser: AdUser) {
		if (!window.confirm(`¿Eliminar el usuario ${adUser.username}? Esta acción no se puede deshacer.`)) {
			return;
		}
	...
	return (
		...
		>
			<section className="page">
				...
				{canManage && (
					<UserFormModal open={modalOpen} user={editingUser} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} />
				)}
			</section>
		</RoleGate>
	);
